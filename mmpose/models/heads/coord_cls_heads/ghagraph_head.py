...
from mmpose.models.utils.ghagcn_block import GHAGCNBlockModule
...
class GHAgraphHead(RTMCCHead):
    def __init__(
        ...
        **kwargs
        ...
        self.cls_gx = nn.Linear(gcn_cfg['hidden_dim'], W, bias=False)
        self.cls_gy = nn.Linear(gcn_cfg['hidden_dim'], H, bias=False)
        ...
        self.gcn_block = GHAGCNBlockModule(
            gcn_cfg, keypoint_connections
        )

    def forward(self, feats):

        flattened_feats = [torch.flatten(f, start_dim=2) for f in feats]
        
        # Concatenate them along the last dimension (axis 2)
        feats = torch.cat(flattened_feats, dim=2)
        feats = self.mlp_GCN(feats)  # -> B, K, hidden
        feats = self.gau(feats)
        feats = self.gcn_block(feats)

        pred_x = self.cls_gx(feats)
        pred_y = self.cls_gy(feats)
